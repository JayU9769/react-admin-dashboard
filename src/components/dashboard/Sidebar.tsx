...
const Index: React.FC = () => {
  ...
  return <aside
    ...
    <nav className={`flex flex-col ${isCollapsed ? 'items-center px-2' : ''} gap-4 sm:py-4`}>
      <Link
        to={"/admin"}
        className={`group flex ${isCollapsed ? '' : 'ms-2'}  shrink-0 items-center justify-center gap-2 rounded-full bg-primary text-lg font-semibold text-primary-foreground md:h-8 md:w-8 md:text-base`}
      >
        <Package2 className="h-4 w-4 transition-all group-hover:scale-110"/>
        <span className="sr-only">Acme Inc</span>
      </Link>
      <Button size="icon" className="p-1 h-6 w-6 rounded-full absolute top-[45px] right-[-12px]"
              onClick={() => dispatch(setIsCollapsed(!isCollapsed))}>
        {isCollapsed ? <ChevronsRight className="transition-all group-hover:scale-110 rounded-full"/> :
